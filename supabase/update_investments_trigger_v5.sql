-- ==========================================================
-- TRIGGER V5: CREDITAR APENAS LUCRO NO SALDO DE SAQUE (BALANCE)
-- ==========================================================

-- 1. Drop old function and recreate
...
BEGIN
    ...
    IF TG_OP = 'UPDATE' AND NEW.status = 'paid' AND (OLD.status IS NULL OR OLD.status <> 'paid') THEN
        ...
        IF r_purchase.id IS NOT NULL THEN
            ...
            -- Update wallet (Apenas v_investor_profit é somado ao balance para saque)
            INSERT INTO wallets (profile_id, balance, future_receipts)
            VALUES (r_purchase.profile_id, v_investor_profit, 0)
            ON CONFLICT (profile_id) DO UPDATE
            SET balance = wallets.balance + v_investor_profit,
        ...
        ELSIF v_investor_id IS NOT NULL THEN
            ...
            -- Update wallet (Apenas v_investor_profit é somado ao balance para saque)
            INSERT INTO wallets (profile_id, balance, future_receipts)
            VALUES (v_investor_id, v_investor_profit, 0)
            ON CONFLICT (profile_id) DO UPDATE
            SET balance = wallets.balance + v_investor_profit,
                future_receipts = GREATEST(0, wallets.future_receipts - v_investor_amortization),
                updated_at = NOW();
        ...
        ELSIF v_device_lot_id IS NOT NULL THEN
            ...
            LOOP
                ...
                -- Update wallet (Apenas v_investor_profit é somado ao balance para saque)
                INSERT INTO wallets (profile_id, balance, future_receipts)
                VALUES (quota_record.profile_id, v_investor_profit, 0)
                ON CONFLICT (profile_id) DO UPDATE
                SET balance = wallets.balance + v_investor_profit,
                    future_receipts = GREATEST(0, wallets.future_receipts - v_investor_amortization),
                    updated_at = NOW();
    ...
    ELSIF (TG_OP = 'UPDATE' AND OLD.status = 'paid' AND (NEW.status IS NULL OR NEW.status <> 'paid')) 
       OR (TG_OP = 'DELETE' AND OLD.status = 'paid') THEN
        ...
        -- Abater das carteiras APENAS transações de Lucro (PROFIT / interest_portion)
        FOR quota_record IN
            SELECT id, profile_id, amount, type, interest_portion FROM wallet_transactions WHERE installment_id = OLD.id
        LOOP
            IF quota_record.type = 'PROFIT' OR COALESCE(quota_record.interest_portion, 0) > 0 THEN
                UPDATE wallets
                SET balance = GREATEST(0, balance - COALESCE(quota_record.interest_portion, quota_record.amount)),
                    updated_at = NOW()
