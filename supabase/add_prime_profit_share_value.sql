-- 1. Add prime_profit_share_value column to devices table
ALTER TABLE devices ADD COLUMN IF NOT EXISTS prime_profit_share_value DECIMAL(12, 2);

-- 2. Drop old function and recreate to support both percentage and fixed profit share
...
BEGIN
    ...
    IF TG_OP = 'UPDATE' AND NEW.status = 'paid' AND (OLD.status IS NULL OR OLD.status <> 'paid') THEN
        ...
        IF v_sale_device_id IS NOT NULL THEN
            SELECT cost_price, sale_price, lot_id, investor_id, prime_profit_share, prime_profit_share_value, prime_admin_fee, prime_valuation_type 
            INTO v_device_cost_price, v_device_sale_price, v_device_lot_id, v_investor_id, v_prime_profit_share, v_prime_profit_share_value, v_prime_admin_fee, v_prime_valuation_type
